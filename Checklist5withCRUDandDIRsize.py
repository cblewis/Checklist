...
theLabelhpsdashboard = Label(root, text="HPS Dashboard:")
...
def uhcresponsecheck():
    ...
    os.startfile(path6)
    os.path.join(path6)
button7 = Button(text="Check UHC Response File", command=uhcresponsecheck)
...
CRUDLabelPrompt2.grid(row=6, column=4, sticky=W)

entryField2 = Entry(root)
entryField2.grid(row=7, column=4, columnspan=3, sticky=W)
...
buttoncrud.grid(row=8, column=4, sticky=W)

DirLabel = Label(root, text="Paste a directory below to get it and its subdirectories' total size (it might take a while):")
DirLabel.grid(row=10, column=4, sticky=W)

entryField3 = Entry(root)
entryField3.grid(row=11, column=4, columnspan=3, sticky=W)

def DirectorySize():
    try:
        total_size = 0
        start_path = entryField3.get()
        for path, dirs, files in os.walk(start_path):
            for f in files:
                fp = os.path.join(path, f)
                total_size += os.path.getsize(fp)
        #print("Directory size: " + str(total_size))
        textBox.insert(END, str(entryField3.get()) + " is " + str(total_size) + " bytes large.\n")
    except:
        textBox.insert(END, "ERROR: Either no directory was found or the measuring process failed.\n")

buttoncrud = Button(root, text="Press this button to measure the directory's total size", command=DirectorySize)
buttoncrud.grid(row=12, column=4, sticky=W)
